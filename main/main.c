...
void start_ota(struct homie_handle_s *handle, int node, int property,
...
static esp_err_t mqtt_event_handler(esp_mqtt_event_handle_t event)
{
    ...
    case MQTT_EVENT_DATA:
        ESP_LOGD(TAG, "MQTT_EVENT_DATA, topic %*.s", event->topic_len,
                 event->topic);
...
static esp_err_t wifi_event_handler(void *ctx, system_event_t *event)
{
    ...
    case SYSTEM_EVENT_STA_GOT_IP:
        ...
        start_remote_log(CONFIG_REMOTELOG_UDP_HOST, CONFIG_REMOTELOG_UDP_PORT,
                         CONFIG_REMOTELOG_SYSLOG_HOST,
                         CONFIG_REMOTELOG_SYSLOG_PORT,
                         CONFIG_REMOTELOG_SYSLOG_APP);
               ...
               const char *data, int data_len)
{
    if (data_len > 0)
    {
        ...
        sprintf(buf_topic, "homie/%s/%s/update/set", homie.deviceid,
                homie.nodes[0].id);
        esp_mqtt_client_publish(homie.mqtt_client, buf_topic, "", 0, 1,
                                HOMIE_TRUE);
        if (ret == ESP_OK)
        {
...
void app_main(void)
{
    ...
    strncpy(homie.firmware_version, app_desc->version,
            sizeof(homie.firmware_version));
    ...
    for (int i = 24 * 60 * 60 / 5; i >= 0; i--)
    {
        ...
        uxBits = xEventGroupWaitBits(mqtt_event_group, MQTT_NEW_CONNECT_BIT,
                                     true, false, 1);
        ...
        uxBits = xEventGroupWaitBits(mqtt_event_group, MQTT_CONNECTED_BIT,
                                     false, false, 1);
